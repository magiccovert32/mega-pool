<div class="row">
	<div class="col-md-12">
		<div class="main-card mb-3 card">
			<div class="card-body">
				<?php if($user_details){ ?>
					<h5 class="card-title">Add player information <small class="form-text text-danger">All fields are mandatory</small></h5>
					
					<form id="update-profile" action="<?php echo base_url('admin-update-player'); ?>" method="post" enctype="multipart/form-data">
						...
						<div class="position-relative form-group">
							<label for="full_name" class="">Player Name</label>
							<input value="<?php echo $user_details['full_name']; ?>" name="full_name" id="full_name" placeholder="Player name" type="text" class="form-control" required="required" autocomlete="off">
						</div>
						<div class="position-relative form-group">
							<label for="user_email" class="">Player Email</label>
							<input value="<?php echo $user_details['user_email']; ?>" name="user_email" id="user_email" placeholder="Player email" type="email" class="form-control" required="required" autocomlete="off">
						</div>
						...
						<button class="mt-1 btn btn-primary" type="submit">Update Player</button>
						<a href="<?php echo  base_url('admin-player-management'); ?>">
							<button class="mt-1 btn btn-outline-dark ml-3" type="button">Cancel</button>
						</a>
					</form>
					
					<?php
						if($this->session->flashdata('player_item')) {
							$message = $this->session->flashdata('player_item');
					?>
						<br/>
