...
class Player extends CI_Controller {
	...
	/**
	 *
	 * Function used to display player list page
	 *
	 */
	public function admin_player_management(){
		$this->admin_template->set('title', 'Players Management');
		$this->admin_template->set('header', 'Players Management');
		$this->admin_template->set('action', 'admin_player_management');
		$this->admin_template->set('page_icon', 'pe-7s-note2');
		
		$data = array();
		$config["base_url"] 		= base_url() . "admin-player-management";
		$config["total_rows"] 		= $this->Usermaster_model->getTotalUserCountForAdmin(2);
		...
		$data["player_list"] 		= $this->Usermaster_model->getAllUserForAdmin(2,$page,$config["per_page"]);
		...
		$this->admin_template->load('admin_template', 'contents' , 'admin/player/admin_player_management', $data);
	}
	 ...
	 *
	 */
	public function admin_update_player(){
		if($this->input->post()){
			...
			if($full_name != '' && $user_email != ''){
				if($this->Usermaster_model->checkEmailExistsWithOutUserId($user_email,$user_id)){
					...
					redirect(base_url('admin-edit-player/'.$user_id)); 
				}else{
					...
					if($this->Usermaster_model->update($playerData,$user_id)){
						...
						redirect(base_url('admin-player-management')); 
					}else{
						$this->session->set_flashdata('item', array('message' => 'Something went wrong, while saving information. Please try again later.','class' => 'danger'));
						redirect(base_url('admin-edit-player/'.$user_id)); 
					}
	 ...
	 *
	 */
	public function admin_edit_player(){
		$data = array();

		$this->admin_template->set('title', 'Edit Player');
		$this->admin_template->set('header', 'Edit Player');
		$this->admin_template->set('action', 'admin_player_management');
		...
		$this->admin_template->load('admin_template', 'contents' , 'admin/player/admin_edit_player', $data);
	}
}
